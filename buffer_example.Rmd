...
Use the `redshift_connect()` function to connect to Redshift. You will need to set your credentials as environment variables first.
...
This could be very helpful to write model results back into Redshift, but we will just use some dummy data in this particular example. Use caution with this one! In addition, the process is slow, especially for lots of data. The function "batches" the dataframe and writes it in incrementally with a `INSERT INTO
